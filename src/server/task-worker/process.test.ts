...
import { AlternateFileType, emptyMetadata, MEDIA_THUMBNAIL_SIZES } from "../../model";
...
test("Process image metadata", async (): Promise<void> => {
  ...
  await expect(
    user1Db.listAlternateFiles(media.id, AlternateFileType.Reencode),
  ).resolves.toInclude([{
    bitRate: null,
    duration: null,
  ...
  await expect(
    user1Db.listAlternateFiles(media.id, AlternateFileType.Thumbnail),
  ).resolves.toInclude([{
    bitRate: null,
    duration: null,
...
test("Reprocess old version", async (): Promise<void> => {
  ...
  await expect(
    user1Db.listAlternateFiles("media1", AlternateFileType.Thumbnail),
  ).resolves.toInclude([{
    id: "alternate3",
    fileName: "alt3.jpg",
    fileSize: 100,
  ...
  await expect(
    user1Db.listAlternateFiles("media1", AlternateFileType.Reencode),
  ).resolves.toInclude([{
    id: "alternate1",
    fileName: "alt1.jpg",
    fileSize: 100,
